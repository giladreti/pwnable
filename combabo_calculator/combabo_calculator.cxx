...
pToken Lexer::readDigit()
{
    ...
    while (isdigit(input[pos]))
        i = i * 10 + input[pos++] - '0';
    if (i > 256)
        throw "does not support integer bigger than 256";
...
pToken Lexer::readId()
{
    ...
    while (isalnum(input[pos++]))
        if (++limit > 256)
            throw "does not support ID longer than 256";
    if (!strncmp("exit", input.substr(start, pos--).c_str(), 4))
        return new Token(T::EXIT);
...
pToken Lexer::readStr()
{
    ...
    while (input[pos++] != '"')
        if (++limit > 256)
            throw "does not support string longer than 256";
    --pos;
    char *str = (char *)calloc(pos - start + 1, 1);
    if (!str)
        throw "Internal Error";
...
pToken Lexer::nToken()
{
    while (input[pos] == ' ')
        pos++;
    if (pos == input.length())
        return new Token(T::END);
    if (isdigit(input[pos]))
        return readDigit();
    if (isalpha(input[pos]))
        return readId();
    if (input[pos] == '+')
        return new Token(T::PLUS, input[pos++]);
    if (input[pos] == '-')
        return new Token(T::MINUS, input[pos++]);
    if (input[pos] == '*')
        return new Token(T::MUL, input[pos++]);
    if (input[pos] == '/')
        return new Token(T::DIV, input[pos++]);
    if (input[pos] == '(')
        return new Token(T::LP, input[pos++]);
    if (input[pos] == ')')
        return new Token(T::RP, input[pos++]);
    if (input[pos] == '"')
        return readStr();
    if (input[pos] == '=')
        return new Token(T::ASSIGN, input[pos++]);
    throw "Lexer Error";
};
...
sToken Calc::visitBinaryOp(sNode node)
{
    ...
    if (leftToken->type == T::INT)
        left = leftToken->value;
    else if (leftToken->type == T::STR)
        left = atoi(leftToken->str);
    else
        throw "Syntax Error";
    if (rightToken->type == T::INT)
        right = rightToken->value;
    else if (rightToken->type == T::STR)
        right = atoi(rightToken->str);
    else
        throw "Syntax Error";
    switch (node->token->type)
    {
    ...
    if (auto variable = visitId(var))
    {
        if (variable->type == T::STR && value->type == T::STR)
        {
            if (variable->size < value->size)
                variable->str = (char *)realloc(variable->str, value->size + 1);
            if (!variable->str)
                throw "Internal Error";
        ...
        else if (variable->type == T::INT && value->type == T::STR)
            variable->value = atoi(value->str);
        else
